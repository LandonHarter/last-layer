...
A trainer for memorizing Rubik's cube F2L (41), OLL (57) and PLL (21) algorithms, plus a speedcubing timer. Each case has several algorithms; you pick your main one or add your own. The header switches between Algorithms mode (Library, Drill) and Timer mode (Timer); Analyze charts and tests the data of either. It is built with Next 16 (App Router), React 19, Tailwind 4, shadcn (base-nova / Base UI) and bun. There is no backend: progress, solves and the drill log are kept in `localStorage`.
...
- `src/lib/setup.ts`: `makeSetup` builds drill setups that are not the inverse of your main: a random LL alg or PLL plus an undone alternative (F2L, OLL), or two other PLLs that combine into the case (PLL). Every setup is checked with the simulator.
...
- `src/lib/scramble.ts`: random-move scrambles (no repeated face, no R L R) and the unfolded net for the preview
